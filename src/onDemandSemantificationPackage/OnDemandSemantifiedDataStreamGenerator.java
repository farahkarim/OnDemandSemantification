...
public class OnDemandSemantifiedDataStreamGenerator extends RdfStream implements Runnable {

	protected final Logger logger = LoggerFactory
			.getLogger(OnDemandSemantifiedDataStreamGenerator.class);
	private boolean keepRunning = false;
	private static long systemTime;
	private static RdfQuadruple q;

	private static WebsocketClientReceivingNodeRedonDemandData clientEndPoint = null;

	public OnDemandSemantifiedDataStreamGenerator(final String iri) {
		super(iri);
	...
	public void run() {
		...
		logger.info("Stream Generator Staretd");
		keepRunning = true;
		try {
			clientEndPoint = new WebsocketClientReceivingNodeRedonDemandData(new URI("ws://127.0.0.1:1880/test2"));
		...
		while (keepRunning) {
			...
			clientEndPoint.addMessageHandler(new WebsocketClientReceivingNodeRedonDemandData.MessageHandler() {
